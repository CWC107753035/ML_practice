...
def plot_pred(dv_set, model, device, lim=350., preds=None, targets=None):
    ...
    figure(figsize=(5, 5))
    plt.scatter(targets*10, preds*10, c='r', alpha=0.5)
...
class COVID19Dataset(Dataset):
    ...
    def __init__(self, path, mode='train', target_only=False):
        ...
        with open(path, 'r') as fp:
            ...
            transformed_data = []
            for row in data[1:]:
    # Transform the numerical values to floats
                transformed_row = [float(val) for val in row[1:]]

                # Transform gender to floats (0 for 'F', 1 for 'M')
                if row[0] == 'male':
                    transformed_row.insert(0, 0.0)
                elif row[0] == 'female':
                    transformed_row.insert(0, 1.0)
        ...
        if not target_only:
            ...
            #feats = list(range(58))
            #feats = list(range(93))
            #feats = list(range(40)) + [46,50,51,64,68,69,57,75,82,86,87]   #state + work + tested  #5
...
plot_learning_curve( model_loss_record , "test")
